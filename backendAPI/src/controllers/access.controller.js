...
class AccessController {
  ...
  signUpByShop = async (req, res, next) => {
    ...
    new CreateRequestSuccess({
      messgage: "Shop sign-up successful",
      metadata: ob,
    }).send(res);
  };
  ...
  signUpByUser = async (req, res, next) => {
    ...
    new CreateRequestSuccess({
      messgage: "User sign-up successful",
      metadata: ob,
    }).send(res);
  };
  getAuthUserInfor = async (req, res, next) => {
    const data = await UserService.findById({ userId: req.user.ob.id });
    new SuccessResponse({ metadata: data, message: "Get user infor" }).send(
      res
    );
  };
